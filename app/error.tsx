  ...
  return (
    <div className="mx-auto max-w-3xl px-5 py-12 fade-up">
      ...
      <div className="mt-4 card border-diag/40 bg-diag/5 p-4">
        ...
        {error.digest && <p className="mt-1 text-[13px] text-ink-faint">digest: {error.digest}</p>}
        {error.stack && (
          <pre className="mt-3 max-h-72 overflow-auto whitespace-pre-wrap break-words text-[13px] leading-relaxed text-ink-dim">
            {error.stack}
          </pre>
        )}
      </div>
